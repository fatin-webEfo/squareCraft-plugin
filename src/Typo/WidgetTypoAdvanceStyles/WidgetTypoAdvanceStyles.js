...
export function opacityinitEffectAnimationDropdownToggle(getSelectedElement) {
  const root = document.getElementById("Typo-advance-opacity-section");
  if (!root) return;

  const arrow = root.querySelector("#Typo-opacity-effect-animation-type-arrow");
  const list = root.querySelector("#Typo-opacity-effect-animation-type-list");
  const display = root.querySelector("#Typo-opacity-effect-animation-value");
  if (!arrow || !list || !display) return;
...
export function opacityinitTypoAdvanceStyles(getSelectedElement) {
  ...
  const defaults = {
    "--sc-Typo-opacity-scroll-start": 0,
    "--sc-Typo-opacity-scroll-end": 100,
    "--sc-Typo-opacity-scroll-entry": 100,
    "--sc-Typo-opacity-scroll-center": 100,
    "--sc-Typo-opacity-scroll-exit": 100,
  };
