(async function squareCraft() {
  ...
  async function createWidget(clickedBlock) {
    ...
    try {
      ...
      if (typeof htmlString === "string" && htmlString.trim().length > 0) {
        await loadWidgetFromString(
          htmlString,
          clickedBlock || pendingClickedBlock
        );
        setTimeout(() => {
  ...
  async function toggleWidgetVisibility(event, clickedBlock = null) {
    ...
    if (!widgetContainer) {
      widgetReadyPromise ||= (async () => {
        await createWidget(clickedBlock || pendingClickedBlock);
    ...
    const targetBlock = clickedBlock || pendingClickedBlock;
    if (targetBlock) {
      try {
        await waitForElement(
          "#typoSection, #imageSection, #buttonSection",
          4000,
          widgetContainer || document
        );
        handleAndDetect(targetBlock);
      } catch (err) {
  ...
  document.body.addEventListener("click", (e) => {
    ...
    const clickedBlock = e.target.closest?.('[id^="block-"]') || null;
    if (clickedBlock) pendingClickedBlock = clickedBlock;
  ...
  (() => {
    ...
    function makeIcon() {
      ...
      wrap.addEventListener("click", (e) => {
        e.preventDefault();
        e.stopPropagation();
        (HOST_WIN.__sc_toggleQueue ||= []).push([null, pendingClickedBlock]);
        const fn =
  ...
  if (Array.isArray(q) && q.length) {
    ...
    q.splice(0).forEach((args) =>
      fn(...(Array.isArray(args) ? args : [null, pendingClickedBlock]))
    );
  }
  ...
  document.body.addEventListener("click", (event) => {
    ...
    if (clickedBlock) {
      pendingClickedBlock = clickedBlock;
      waitForElement(
        "#typoSection, #imageSection, #buttonSection",
        4000,
        widgetContainer || document
  ...
  async function loadWidgetFromString(htmlString, clickedBlock) {
    ...
    if (targetBlock) {
      try {
        ...
        detectBlockElementTypes(targetBlock);
      } catch (error) {
        console.error(error.message);
      }
    }
