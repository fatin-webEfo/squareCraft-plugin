  ...
  window.addEventListener("load", async () => {
    ...
    async function saveModifications(elementId, css) {
      if (!pageId || !elementId || !css) {
        ...
        // Ensure font-size is stored if applied
        if (css["font-size"]) {
            let fontSizeInput = document.getElementById("squareCraftFontSize");
            if (fontSizeInput) {
                fontSizeInput.value = parseInt(css["font-size"], 10); // Sync UI
            }
        }
...
fontfamilies();
  ...
  function attachEventListeners() {
    ...
    const fontStyleUndoButton = document.querySelector(".squareCraft-rounded-6px.squareCraft-rotate-180.squareCraft-px-1_5.squsareCraft-font-style.squareCraft-cursor-pointer");
