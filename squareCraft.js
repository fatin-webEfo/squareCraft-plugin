...
function applyStylesToElement(element, css) {
    ...
    Object.keys(css).forEach((prop) => {
        element.style.setProperty(prop, css[prop], "important"); // Applying styles with !important
    });
}
...
document.body.addEventListener("click", (event) => {
    ...
    if (alignmentIcon && lastClickedElement) {
        ...
        if (lastAppliedAlignment === textAlign) {
            // Remove alignment from both element and nested elements
            applyStylesToElement(lastClickedElement, { "textAlign": "" });

            const nestedElements = lastClickedElement.querySelectorAll("h1, h2, h3, h4, p");
            nestedElements.forEach(nestedElement => applyStylesToElement(nestedElement, { "textAlign": "" }));

            lastAppliedAlignment = null;
            console.log(`❌ Alignment undone for Block: ${lastClickedBlockId}`);
        } else {
            // Apply alignment to both element and nested elements
            applyStylesToElement(lastClickedElement, { "textAlign": textAlign });

            const nestedElements = lastClickedElement.querySelectorAll("h1, h2, h3, h4, p");
            nestedElements.forEach(nestedElement => applyStylesToElement(nestedElement, { "textAlign": textAlign }));
